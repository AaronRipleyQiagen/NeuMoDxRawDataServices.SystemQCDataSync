...
def Add_Dash(app):
    ...
    def upload_tadm_image_to_blob_storage(list_of_contents, list_of_filenames, runset_selection, runset_review, is_open):
        if list_of_contents:
            files = {list_of_filenames[i]: list_of_contents[i]
                     for i in range(len(list_of_filenames))}
            upload_status = []
    ...
    @app.callback(Output('tadm-pictures-table', 'rowData'),
    ...
    def get_tadm_picture_table(active_tab, message_children, is_open, runset_data):
        if ((ctx.triggered_id == 'upload-tadm-message') or ctx.triggered_id == 'review-tabs' or (ctx.triggered_id == 'delete-tadm-picture-response' and is_open == False)) and active_tab == 'tadm-pictures':
            """
            Get TADM Picture Info from API 
            """
            tadm_pictures_url = os.environ['RUN_REVIEW_API_BASE'] + \
            ...
            for column in tadm_picture_data.columns:
                ...
                if 'Date' in column:
                    tadm_picture_data[column] = tadm_picture_data[column].astype(
                        'datetime64').dt.strftime("%d %B %Y %H:%M:%S")
    ...
    @app.callback(Output('delete-tadm-picture-button', 'disabled'),
                  Input('tadm-pictures-table', 'selectionChanged'))
    ...
    @app.callback(Output('delete-tadm-picture-confirmation', 'is_open'),
                  ...
                  prevent_intitial_call=True)
    def control_delete_tadm_picture_popup(delete_click, confirm_click, cancel_click, is_open):
        if 'delete' in ctx.triggered_id:
            return not is_open
        return is_open

    @app.callback(Output('delete-tadm-picture-response', 'is_open'),
                  Input('delete-tadm-picture-confirm', 'n_clicks'),
    ...
    def delete_tadm_picture(confirm_click, selection, is_open):
        if ctx.triggered_id == 'delete-tadm-picture-confirm':
            delete_tadm_picture_url = os.environ['RUN_REVIEW_API_BASE'] + \
