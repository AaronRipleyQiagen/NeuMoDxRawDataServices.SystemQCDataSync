...
def update_modules():
    modules_update = {}
    module_results = requests.get("https://localhost:7107/api/xpcrmodules", verify=False).json()
...
def getXPCRModuleCartridges(module_id):
    request_url = "https://localhost:7107/api/xpcrmodules/{}/xpcrmoduleconfigurations".format(module_id)
    
    module_data = pd.DataFrame(columns=['Run Start Time', 'Run End Time', '# of Samples', 'Assays'])
    module_data.index.names = ['id']
    
    def cartridgeToDataFrame(cartridge):
        module_data.loc[cartridge['id'], ['Run Start Time', 'Run End Time', '# of Samples', 'Assays']] = [cartridge['validFrom'], cartridge['validTo'], cartridge['sampleCount'], cartridge['assays']]
    
    xpcrModule = requests.get("https://localhost:7107/api/xpcrmodules/{}/xpcrmoduleconfigurations".format(module_id), verify=False).json()
    cartridge_samples = {}
    for xpcrconfiguration in xpcrModule['xpcrModuleConfigurations']:
        
        xpcrconfiguration = requests.get("https://localhost:7107/api/xpcrmoduleconfigurations/{}/cartridges".format(xpcrconfiguration['id']), verify=False).json()
        
        for cartridge in xpcrconfiguration['cartridges']: 
            
            cartridge_details = requests.get("https://localhost:7107/api/cartridges/{}/samples/assays".format(cartridge['id']), verify=False).json()
            sample_ids = []
            cartridge_assays = []
    ...
    module_data['Run Start Time'] = module_data['Run Start Time'].astype('datetime64').dt.strftime("%d%B%Y %H:%M:%S")
    module_data['Run End Time'] = module_data['Run End Time'].astype('datetime64').dt.strftime("%d%B%Y %H:%M:%S")
    return module_data.reset_index().to_dict('records'), cartridge_samples

module_data = pd.DataFrame(columns=['id','Run Start Time', 'Run End Time', '# of Samples', 'Assays'])
...
layout = html.Div([
    ...
    html.Div([
        ...
        dcc.Loading(id='samples-loading',
                    ...
                    fullscreen=True,
                    children=
                            dash_table.DataTable(id='runs', sort_action='native', row_selectable='multi', columns=[{'name': i, 'id': i, 'deletable': True} for i in module_data.columns
                            # omit the id column
                            if i != 'id'
                            ]))
    ]),
    html.Div([
        dbc.Button('Get Data', id='view-run-data', n_clicks=0, href="/run-review/results"),
    ])
