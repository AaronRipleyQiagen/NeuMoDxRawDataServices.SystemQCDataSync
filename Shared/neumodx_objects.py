...
class nmdx_file_parser:
    ...
    def readChannelData(file, sheet, channel):
        ...
        if len(channelData_all) > 0:
            ChannelRawStart = (
                channelData_all[channelData_all["Sample ID"] == "Raw"].index.values[0]
                + 1
            )
            ChannelRawEnd = (
                channelData_all[
                    channelData_all["Sample ID"] == "Normalized"
                ].index.values[0]
                - 2
            )
            ...
            ChannelNormEnd = (
                channelData_all[
                    channelData_all["Sample ID"] == "SecondDerivative"
                ].index.values[0]
                - 2
            )
            ...
            if "Modulated" in channelData_all["Sample ID"].unique():
                Channel2ndEnd = (
                    channelData_all[
                        channelData_all["Sample ID"] == "Modulated"
                    ].index.values[0]
                    - 2
                )
                ChannelModulatedStart = (
                    channelData_all[
                        channelData_all["Sample ID"] == "Modulated"
                    ].index.values[0]
                    + 1
                )
                ChannelModulated = channelData_all.loc[
                    ChannelModulatedStart : ChannelModulatedStart + len(ChannelRaw)
                ]
                ...
                if (
                    len(ChannelRaw) == len(ChannelNorm)
                    and len(ChannelRaw) == len(Channel2nd)
                    and len(ChannelRaw) == len(ChannelModulated)
                ):
                ...
                Channel2nd = channelData_all.loc[
                    Channel2ndStart : Channel2ndStart + len(ChannelRaw)
                ]
                Channel2nd["Processing Step"] = "2nd"
    ...
    def readRawData(file):
        ...
        Summary_COC_Data = Summary_Tab.set_index(
            ["Test Guid", "Replicate Number"]
        ).join(
            COC_Tab.set_index(["Test Guid", "Replicate Number"]).loc[
                :, [x for x in COC_Tab.columns if x not in Summary_Tab.columns]
            ]
        )
        ...
        channelDataFinal = pd.concat(
            [
                channelDataDict[df]
                for df in channelDataDict
                if len(channelDataDict[df]) > 0
            ],
            axis=0,
        )

        channelDataFinal.set_index(
            ["Target Result Guid", "Processing Step", "Channel"],
            append=True,
            inplace=True,
        )
        ...
        channelDataFinal_summary = (
            channelDataFinal.swaplevel(3, 0).swaplevel(3, 1).swaplevel(3, 2)
        )
    ...
    def scrapeFile(self, file, filename):
        ...
        for col in channel_summary.columns:
            if "Barcode" in col:
                ...
                channel_summary[col] = channel_summary[col].replace("nan", np.nan)
        channel_summary = channel_summary.astype(object).where(
            pd.notna(channel_summary), None
        )
        ...
        for col in summary_coc.loc[
            :, [col for col in summary_coc if "Date" in col]
        ].columns:
            summary_coc[col] = pd.to_datetime(summary_coc[col], utc=False).apply(
                lambda x: x.replace(tzinfo=pytz.utc)
            )

        channel_readings = channel_readings.astype(object).where(
            pd.notna(channel_readings), None
        )
        ...
        flat_data = summary_coc.join(
            channel_summary.loc[
                :, [x for x in channel_summary.columns if x not in summary_coc.columns]
            ]
        ).join(
            channel_readings.loc[
                :,
                [
                    x
                    for x in channel_readings.columns
                    if x not in channel_summary.columns
                ],
            ]
        )
...
class api_base:
    ...
    def get_unique_objects(self):
        ...
        unique_data = self.data.drop_duplicates(subset=[x for x in self.properties])
    ...
    def postData(self):
        ...
        self.post_objects()
        return (
            self.data.set_index([x for x in self.properties])
            .join(self.responses)
            .reset_index()
        )
...
class Assay(api_base):
    ...
    def __init__(self, data):
        ...
        self.data["Dilution Factor"] = self.data["Dilution Factor"].astype(np.float32)
...
class NeuMoDxSystem(api_base):
    ...
    properties = {
        "N500 Serial Number": "n500SerialNumber",
...
class XPCRModuleConfiguration(api_base):
    properties = {
        "XPCRFirmwareApplicationVersion": "xpcrFirmwareApplicationVersion",
    ...
    def __init__(self, data):
        ...
        self.data[
            [
                "XPCRFirmwareApplicationVersion",
                "XPCRFirmwareBootloaderVersion",
                "ABFirmwareApplicationVersion",
                "ABFirmwareBootloaderVersion",
                "XPCRConfigurationStartDate",
                "XPCRConfigurationEndDate",
            ]
        ] = np.nan
        self.data["XPCR Firmware Version"].fillna("App: Null, Bolo: Null", inplace=True)
        self.data["XPCR Actuator Version"].fillna("App: Null, Bolo: Null", inplace=True)
        self.data[
            ["XPCRFirmwareApplicationVersion", "XPCRFirmwareBootloaderVersion"]
        ] = [
            [x.replace("App: ", ""), y.replace(" Bolo: ", "")]
            for x, y in self.data["XPCR Firmware Version"].str.split(",")
        ]
        self.data[["ABFirmwareApplicationVersion", "ABFirmwareBootloaderVersion"]] = [
            [x.replace("App: ", ""), y.replace(" Bolo: ", "")]
            for x, y in self.data["XPCR Actuator Version"].str.split(",")
        ]
        for col in [
            "XPCRFirmwareApplicationVersion",
            "XPCRFirmwareBootloaderVersion",
        ...
        self.data["XPCR Module Index"] = self.data["XPCR Module Index"].astype("Int64")
...
class HeaterModuleConfiguration(api_base):
    ...
    properties = {
        "HeaterFirmwareApplicationVersion": "heaterFirmwareApplicationVersion",
    ...
    def __init__(self, data):
        ...
        self.data[
            [
                "HeaterFirmwareApplicationVersion",
                "HeaterFirmwareBootloaderVersion",
                "HeaterConfigurationStartDate",
                "HeaterConfigurationEndDate",
            ]
        ] = np.nan
        self.data[
            ["HeaterFirmwareApplicationVersion", "HeaterFirmwareBootloaderVersion"]
        ] = [
            [x.replace("App: ", ""), y.replace(" Bolo: ", "")]
            for x, y in self.data["Heater Firmware Version"].str.split(",")
        ]
...
class AssayChannel(api_base):
    ...
    properties = {"Channel": "channel", "Target Name": "targetName"}
    # 'AssayId':'assayId',}
...
class Sample(api_base):
    ...
    def __init__(self, data):
        ...
        self.data["Replicate Number"] = self.data["Replicate Number"].astype("Int64")
...
class ChannelSummarySet(api_base):
    ...
    def create_objects(self):
        ...
        # Make sure all Columns are available before Proceeding
        self.data[[x for x in self.properties if x not in self.data.columns]] = np.nan
        self.data.drop_duplicates(subset=[x for x in self.properties], inplace=True)
        ...
        for item in self.data.index.unique():
            ...
            object["AssayChannel"] = AssayChannel(self.data.loc[[item]]).create_object()
...
class CrossTalkSetting(api_base):
    ...
    properties = {
        "Green Crosstalk Into Yellow": "greenIntoYellow",
...
class TestStripMapIVD(TestStripMap):
    ...
    properties = {
        "Test Strip NeuMoDx Carrier": "carrier",
    ...
    def __init__(self, data):
        self.data = data.copy()
        self.data["Test Strip NeuMoDx Carrier"] = (
            self.data["Test Strip NeuMoDx Carrier"]
            .str.replace("TestStrips", "")
            .fillna(np.nan)
            .astype("Int64")
        )
        self.data["Test Strip NeuMoDx Carrier"] = np.where(
            self.data["Test Strip NeuMoDx Carrier"].isnull(),
            np.nan,
            self.data["Test Strip NeuMoDx Carrier"],
        )
...
class TestStripMapMM(TestStripMap):
    ...
    properties = {
        "Test Strip LDT Master Mix Carrier": "carrier",
    ...
    def __init__(self, data):
        self.data = data.copy()
        self.data["Test Strip LDT Master Mix Carrier"] = (
            self.data["Test Strip LDT Master Mix Carrier"]
            .str.replace("TestStrips", "")
            .fillna(np.nan)
            .astype("Int64")
        ...
        self.data["Test Strip LDT Master Mix Carrier"] = np.where(
            self.data["Test Strip LDT Master Mix Carrier"].isnull(),
            np.nan,
            self.data["Test Strip LDT Master Mix Carrier"],
        )
...
class TestStripMapPPM(TestStripMap):
    ...
    properties = {
        "Test Strip LDT Primer Carrier": "carrier",
    ...
    def __init__(self, data):
        self.data = data.copy()
        self.data["Test Strip LDT Primer Carrier"] = (
            self.data["Test Strip LDT Primer Carrier"]
            .str.replace("TestStrips", "")
            .fillna(np.nan)
            .astype("Int64")
        ...
        self.data["Test Strip LDT Primer Carrier"] = np.where(
            self.data["Test Strip LDT Primer Carrier"].isnull(),
            np.nan,
            self.data["Test Strip LDT Primer Carrier"],
        )
...
class ChainOfCustodySet(api_base):
    ...
    def create_object(self):
        cleanobject = {}
        cleanobject["SampleDefinition"] = SampleDefinition(self.data).create_object()
        cleanobject["LhpATrace"] = LhpATrace(self.data).create_object()
        cleanobject["LysisBindingTrace"] = LysisBindingTrace(self.data).create_object()
        cleanobject["LhpBTrace"] = LhpBTrace(self.data).create_object()
        cleanobject["ExtractionTrace"] = ExtractionTrace(self.data).create_object()
    ...
    def post_objects(self):
        ...
        for object in self.objects:
            ...
            result = self.write_post(self.cleanobject)
            self.responses.loc[
                self.cleanobject["sampleId"], self.responseColumnName
            ] = result.headers[[x for x in result.headers if "Id" in x][0]]

    def postData(self):
        self.get_unique_objects()
        self.post_objects()
        return (
            self.data.set_index(["SampleId"])
            .join(
                self.responses.loc[
                    :, [x for x in self.responses.columns if x not in self.data.columns]
                ]
            )
            .reset_index()
        )
...
class LhpATrace(api_base):
    ...
    def __init__(self, data_item):
        self.data = data_item.copy()
        self.data["Buffer Large Tip Position"] = self.data[
            "Buffer Large Tip Position"
        ].astype("Int64")
        self.data["Sample Large Tip Position"] = self.data[
            "Sample Large Tip Position"
        ].astype("Int64")
        self.data["LHPA Start Date Time"] = self.data["LHPA Start Date Time"].apply(
            lambda x: x.isoformat(timespec="milliseconds")
        )
        self.data["LHPA ADP Position"] = self.data["LHPA ADP Position"].astype(str)
        self.data["Sample Tube Rack"] = self.data["Sample Tube Rack"].astype("Int64")
        self.data["Sample Tube Position"] = self.data["Sample Tube Position"].astype(
            "Int64"
        )
        self.data["Specimen Tube Type"] = self.data["Specimen Tube Type"].astype(str)
...
class LhpBTrace(api_base):
    ...
    def __init__(self, data_item):
        self.data = data_item.copy()
        self.data["LHPB Large Tip Position"] = self.data[
            "LHPB Large Tip Position"
        ].astype("Int64")
        self.data["LHPB Start Date Time"] = np.where(
            self.data["LHPB Start Date Time"].isnull(),
            np.nan,
            self.data["LHPB Start Date Time"].apply(
                lambda x: x.isoformat(timespec="milliseconds")
            ),
        )
        self.data["LHPB ADP Position"] = self.data["LHPB ADP Position"].astype(str)
    ...
    def create_object(self):
        ...
        cleanobject["LhpBLargeTipRack"] = LhpBLargeTipRack(self.data).create_object()
        return cleanobject
...
class ExtractionTrace(api_base):
    ...
    def __init__(self, data_item):
        self.data = data_item.copy()
        self.data["Start Heater Ambient Temperature C"] = self.data[
            "Start Heater Ambient Temperature C"
        ].astype(float)
        self.data["End Heater Ambient Temperature C"] = self.data[
            "End Heater Ambient Temperature C"
        ].astype(float)
        self.data["Bulk Reagent Drawer"] = self.data["Bulk Reagent Drawer"].astype(str)
...
class LhpCTrace(api_base):
    ...
    def __init__(self, data_item):
        ...
        self.data["LHPC Start Date Time"] = np.where(
            self.data["LHPC Start Date Time"].isnull(),
            np.nan,
            self.data["LHPC Start Date Time"].apply(
                lambda x: x.isoformat(timespec="milliseconds")
            ),
        )
        self.data["LHPC ADP Position"] = self.data["LHPC ADP Position"].astype(str)
    ...
    def create_object(self):
        ...
        cleanobject["TestStripMapIVD"] = TestStripMapIVD(self.data).create_object()
        cleanobject["TestStripMapMM"] = TestStripMapMM(self.data).create_object()
        cleanobject["TestStripMapPPM"] = TestStripMapPPM(self.data).create_object()
...
class PcrTrace(api_base):
    ...
    def __init__(self, data_item):
        self.data = data_item.copy()
        self.data["Start Pcr Ambient Temperature C"] = self.data[
            "Start Pcr Ambient Temperature C"
        ].astype(float)
        self.data["End Pcr Ambient Temperature C"] = self.data[
            "End Pcr Ambient Temperature C"
        ].astype(float)
        self.data["PCR Start Date Time"] = np.where(
            self.data["PCR Start Date Time"].isnull(),
            np.nan,
            self.data["PCR Start Date Time"].apply(
                lambda x: x.isoformat(timespec="milliseconds")
            ),
        )
...
class ReadingSet(api_base):
    ...
    def create_objects(self):
        ...
        for item in self.data.index.unique():
            object = self.data.loc[item, self.properties].to_dict()
            object = {k: None if pd.isnull(v) else v for k, v in object.items()}
            object["Readings"] = Readings(self.data.loc[item]).create_readings_list()
    ...
    def post_objects(self):
        self.responses = pd.DataFrame(self.objects).set_index(["AssayChannelStepId"])
        ...
        for object in self.objects:
            ...
            result = self.write_post(self.cleanobject)
            self.responses.loc[
                self.cleanobject["assayChannelStepId"], self.responseColumnName
            ] = result.headers[[x for x in result.headers if "Id" in x][0]]
    ...
    def postData(self):
        ...
        self.post_objects()
        return (
            self.data.set_index(["AssayChannelStepId"])
            .join(
                self.responses.loc[
                    :, [x for x in self.responses.columns if x not in self.data.columns]
                ]
...
class Readings(api_base):
    def __init__(self, data_item):
        self.data = data_item.copy()
        self.data = self.data[["Readings " + str(i) for i in range(1, 200)]].astype(
            float
        )

    def create_readings_list(self):
        readings_array = [
            [int(key.replace("Readings ", "")), value]
            for (key, value) in self.data.to_dict().items()
        ]
        readings_list = [
            {"cycle": item[0], "value": item[1]}
...
class Flag(api_base):
    ...
    def create_objects(self):
        ...
        for item in self.data.index.unique():
            ...
            assaychannel["Assay"] = Assay(self.data.loc[[item]]).create_object()
            ...
            for flag in flags_list:
                ...
                severity, description = flag_string.replace(")", "").split(", ")
                flag_object = {
...
class SampleJson:
    ...
    def create_sample_json(self, sample_data):
        ...
        sample["ExtractionPlate"] = ExtractionPlate(sample_data).create_object()
        ...
        sample["ChainOfCustodySet"] = ChainOfCustodySet(sample_data).create_object()
        sample["ChannelSummarySets"] = ChannelSummarySet(sample_data).create_objects()
